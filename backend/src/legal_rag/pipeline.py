...
from src.legal_rag.chunker import chunk_file
from src.legal_rag.vectorstore import upload_chunks
...
def ingest_batch(directory: Path | str, dry_run: bool = False):
    ...
    logger.info(
        "Generated a total of %d chunks from %d files.", len(all_chunks), len(txt_files)
    )
