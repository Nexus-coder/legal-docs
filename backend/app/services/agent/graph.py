...
from app.services.ingestion.indexer import retrieve_context
...
# The user specifies GPT integrations are configured via OPENAI_API_KEY
...
def draft_node(state: AgentState) -> dict:
    ...
    messages = [
        SystemMessage(content="You are an expert litigation lawyer assisting in drafting Kenyan legal pleadings. Output only the requested pleading block without conversational padding."),
...
def critique_node(state: AgentState) -> dict:
    ...
    messages = [
        SystemMessage(content="You are a strict Senior Partner reviewing a draft. Your standard is absolute perfection."),
        HumanMessage(content=prompt)
    ]
